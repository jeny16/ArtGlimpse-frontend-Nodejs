...
const CartItem = ({ item }) => {
  ...
  // console.log("✅ Products from Redux in CartItem:", products);

  const productDetail = products.find((prod) => prod._id === item.productId)
    || item.productData
    || {};

  const productId = item.productId || productDetail._id || 'unknown-id';
        ...
        <Grid item xs={9} sm={10}>
          <Grid container>
            <Grid item xs={12} md={7}>
              <Typography variant="h6" sx={{ fontWeight: 700, fontSize: '16px', color: theme.palette.custom.highlight || '#282c3f' }}>
                {name}
              </Typography>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <IconButton size="small" onClick={() => handleQuantityChange(quantity - 1)} disabled={quantity <= 1}
                    sx={{ p: 0.5, color: quantity <= 1 ? theme.palette.shades.medium : theme.palette.custom.highlight }}>
                    <RemoveCircleOutlineIcon fontSize="small" />
                  </IconButton>
                  <Typography sx={{ mx: 1, fontWeight: 600, fontSize: '14px', width: '20px', textAlign: 'center' }}>
                    {quantity}
                  </Typography>
            ...
            <Grid item xs={12} md={5} sx={{ textAlign: { xs: 'left', md: 'right' }, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', mb: 1 }}>
                ...
                {originalPrice > finalPrice && (
                  <>
                    <Typography sx={{ textDecoration: 'line-through', color: '#999', fontSize: '14px', mr: 1 }}>
                      ₹{originalPrice}
                    </Typography>
